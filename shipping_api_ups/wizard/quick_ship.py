...
class quick_ship(models.TransientModel):
    _name = "quick.ship"
    _description = "Quick Ship"
    _rec_name = 'saleorder_id'
    
    ship_log_id =       fields.Many2one('shipping.move', string='Ship Move')
    saleorder_id =      fields.Many2one('sale.order', string='Sale Order', required='1')
    sender =            fields.Many2one('res.partner', string='Sender')
    send_addr =         fields.Many2one('res.partner', string='Sender Address')
    recipient =         fields.Many2one('res.partner', string='Recipient')
    address_id =        fields.Many2one('res.partner', string='Address', help="Ship to address")
    quantum_view =      fields.Boolean(string='Send QuantumView Notifications')
    ups_shipper_id =    fields.Many2one('ups.account.shipping', string='Bill to')
    ...
    ups_packaging_type =fields.Many2one('shipping.package.type', string='Package Type',help='Indicates the type of package – taken from \
    ...
    insured_val =       fields.Float(string='Insured Value')
    addn_service =      fields.Boolean(string='Additional Service Options')
    description =       fields.Char(string='Description', size=256,)
    inv_no =            fields.Char(string='Invoice Number', size=64,)
    ref_no =            fields.Char(string='Reference Number', size=64,)
    ref1 =              fields.Selection([
                                    ...
                                    ], string='Reference Code', help='Indicates the type of package')
    shipcharge =        fields.Float(string='Rate')
    sat_delivery =      fields.Boolean(string='Saturday Delivery', help='Indicates is it is appropriate to send delivery on Saturday.')
    verbal_confirm =    fields.Boolean(string='Verbal Confirmation of Delivery')
    con_name =          fields.Char(string='Name', size=128)
    con_phone =         fields.Char('Phone', size=64)
    cod =               fields.Boolean(string='Collect on Delivery (COD)')
    amount =            fields.Float(string='Amount')
    acc_cash_moneyorder=fields.Boolean(string='Accept Cashier Check/Money Order Only')
    shipper_release =   fields.Boolean(string='Shipper Release')
    deliv_conf =        fields.Boolean(string='Delivery Confirmation')
    dc_opt =            fields.Selection([('', ''), ('', '')], string='DC Options')
    deliv_conf =        fields.Boolean('Offset the Climate Impact of This Shipment', help="UPS Carbon Neutral")
    addnl_handling =    fields.Boolean(string='Additional Handling')
    response =          fields.Text(string='Response')
    logo =              fields.Binary(string='Logo')

    @api.multi
    def onchange_sale_id(self, saleorder_id):
        ret = {}
        if saleorder_id:
            sale = self.env['sale.order'].browse(saleorder_id)
            ret.update({
            ...
            ups_shipping_service_ids = service_type_obj.search([('description', 'like', sale.ship_method)])
            if ups_shipping_service_ids:
                ups_shipping_service = service_type_obj.browse(ups_shipping_service_ids)[0]
...
#             PackagingType.appendChild(Description)
        ...
        if quick_ship_id.logis_company.id:
            ...
            raise except_orm(_('No Company Selected!'),_("Please select a logistic company.") )
...
#            'package_weight': quick_ship_id.weight,
        ...
        ship_move_id = self.env['shipping.move'].create(value_dic)
        if err_flag:
            ret = False
        else:
    ...
    @api.multi
    def process_ship_accept(self, ret_res):
        ids=self._ids
        quick_ship_id = self.browse(ids)
        ...
        if len(response_dic['ShipmentAcceptResponse']) > 1 and 'ShipmentResults' in response_dic['ShipmentAcceptResponse'][1]:
            ...
            self.write({'shipcharge': NegotiatedRates or '0', 'logo': label_image})
            if ret_res.get('ship_move_id'):
                self.write({'shipcharge': NegotiatedRates or '0', 'ship_log_id': ret_res.get('ship_move_id')})
                tracking_url = ''
                if TrackingNumber:
                ...
                ship_move_id = self.env['shipping.move'].write(ret_res.get('ship_move_id'), {
                    'logo': label_image,
    ...
    @api.multi
    def get_rate(self):
        res = self.process_ship()
        if res:
            res = self.process_ship_accept(res)
            self.write({'response': ''})
        return True
    ...
    def print_label(self):
        ...
        if not ship_log_id:
            raise except_orm(_('Error!'),_("Sale order is not processed."))
        return {
